...
WLED_GLOBAL uint32_t favoriteFxMask[8] _INIT_N(({0,0,0,0,0,0,0,0})); // bitmap, 1 bit per effect id (0..255); 1 = user-favorite
inline bool isFxFavorite(uint8_t id)           { return (favoriteFxMask[id >> 5] >> (id & 31)) & 1u; }
inline void setFxFavorite(uint8_t id, bool fv) { if (fv) favoriteFxMask[id >> 5] |=  (1u << (id & 31));
                                                 else    favoriteFxMask[id >> 5] &= ~(1u << (id & 31)); }
